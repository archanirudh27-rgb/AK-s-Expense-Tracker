...
function openIncome(){
 let d=document.getElementById('income-modal');if(d){d.remove();return}
 d=document.createElement('div');d.id='income-modal';d.style='position:fixed;inset:0;background:rgba(0,0,0,.45);z-index:100;display:flex;align-items:flex-end;justify-content:center';
 const total=(typeof rows!=='undefined'?rows.filter(x=>x.category==='Income'&&typeof monthOf==='function'&&monthOf(x.date)===(typeof month!=='undefined'?month:monthOf(new Date()))).reduce((s,x)=>s+Number(x.amount),0):0);
 d.innerHTML='<div class="card" style="width:min(680px,100%);margin:0;border-radius:20px 20px 0 0;max-height:90vh;overflow:auto"><div class="row"><div><b>Income</b><div class="muted">This month: ₹'+Math.round(total).toLocaleString('en-IN')+'</div></div><button class="btn secondary" id="income-close">×</button></div><div class="form" style="margin-top:12px"><div class="field"><label>Date</label><input id="income-date" class="input" type="date" value="'+new Date().toISOString().slice(0,10)+'"></div><div class="field"><label>Amount (₹)</label><input id="income-amount" class="input" type="number" min="0.01" step="0.01"></div><div class="field"><label>Income type</label><select id="income-type" class="select">'+TYPES.map(x=>'<option>'+x+'</option>').join('')+'</select></div><div class="field"><label>Description</label><input id="income-desc" class="input" placeholder="e.g. August salary"></div></div><button class="btn gold" style="margin-top:13px;width:100%" id="income-save">Save income</button><div class="section" style="margin-top:18px">Recent income</div><div id="income-list" class="muted" style="margin-top:8px"></div></div>';
 document.body.appendChild(d);document.getElementById('income-close').onclick=()=>d.remove();document.getElementById('income-save').onclick=saveIncome;renderIncomeList();
}
