...
/* v44 boot guard: keep the legacy base UI hidden until current runtime patches are installed. */
(function(){
  if(!document.getElementById('fintrack-v44-boot')){
    const s=document.createElement('style');
    s.id='fintrack-v44-boot';
    s.textContent='html{visibility:hidden!important;background:#0d100d}';
...
(function(){
  ...
  function installSavings(){
    try{
      ...
      function fmtReconciled(s){
        if(!s)return'Not set';
        try{if(s.created_at)return new Date(s.created_at).toLocaleString('en-IN',{day:'2-digit',month:'short',year:'numeric',hour:'2-digit',minute:'2-digit'})}catch(e){}
        return s.date||'—';
      }
      ...
      function savingsHomeCard(){
        ...
        const net=currentMonthNet(),netColor=net>=0?'var(--green)':'var(--red)',flow=d.income-d.expenses-d.investments,flowColor=flow>=0?'var(--green)':'var(--red)';
      ...
      function savingsSettingsCard(){
        const d=liveBalanceData(),list=snapshots().slice(0,5),today=new Date().toISOString().slice(0,10),title=d?'Reconcile current savings':'Set opening savings balance';
      ...
      window.saveSavingsBalance=async function(){
        try{
          const amount=Number(document.getElementById('savingsBalance')?.value),date=document.getElementById('savingsDate')?.value||new Date().toISOString().slice(0,10);
          ...
          await load();if(typeof go==='function')go('home');
        }catch(e){alert('Could not save the savings balance. Please try again.')}
      };
      /* Current layout: savings belongs at the END of Home, not at the top. */
      home=function(){return baseHome()+savingsHomeCard()};
      settings=function(){return savingsSettingsCard()+baseSettings()};
      tx=function(){const all=rows;rows=all.filter(x=>!isBalanceSnapshot(x));try{return baseTx()}finally{rows=all}};
      filterTx=function(){const all=rows;rows=all.filter(x=>!isBalanceSnapshot(x));try{return baseFilterTx()}finally{rows=all}};
  ...
  let tries=0;
  const timer=setInterval(function(){tries++;const a=installTypeCompatibility(),b=installSavings();if((a&&b)||tries>240)clearInterval(timer)},25);
})();

(function(){
  const style=document.createElement('style');
  style.textContent=`body.dark .notice{background:#2b2416!important;border-color:#6f552a!important;color:#f6e7c8!important}body.dark .notice.ok{background:#17301f!important;border-color:#315c3b!important;color:#d9f0dd!important}body.dark .notice.err{background:#351c1b!important;border-color:#6b3632!important;color:#f4d6d2!important}`;
  ...
  function installLayoutV32(){
    try{
      ...
      const previousSettings=settings,previousHome=home;
      nav=function(){
      ...
      home=function(){return previousHome().replace(/<button class="btn secondary" onclick="go\('income'\)">\+ Add income<\/button>/,'')};
      settings=function(){const incomeShortcut='<div class="card"><div class="section">More</div><p class="muted">Occasional actions and account tools.</p><div class="actions" style="margin-top:12px"><button class="btn gold" onclick="go(\'income\')">₹ Log income</button></div></div>';return incomeShortcut+previousSettings()};
      window.__fintrackLayoutV32=true;
      if(typeof user!=='undefined'&&user)render();
      return true;
    }catch(e){return false}
  }
  let tries=0;const timer=setInterval(function(){tries++;if(installLayoutV32()||tries>240)clearInterval(timer)},25);
})();

(function(){
  let cloudOverrides={},patched=false,busy=false,syncCount=0;
  function readMap(key){try{return JSON.parse(localStorage.getItem(key)||'{}')}catch(e){return{}}}
  function localMap(id){return Object.assign({},readMap('ak_expense_types_'+id),readMap('fintrack_types_'+id))}
  function writeLocal(id,map){const data=JSON.stringify(map||{});localStorage.setItem('ak_expense_types_'+id,data);localStorage.setItem('fintrack_types_'+id,data)}
  function currentUser(){try{return (typeof user!=='undefined'&&user)?user:null}catch(e){return null}}
  async function persistCloud(map){const u=currentUser();if(!u||typeof sb==='undefined'||!sb)return false;try{const r=await sb.from('settings').upsert({user_id:u.id,overrides:map||{}},{onConflict:'user_id'});return !r.error}catch(e){return false}}
  function installCloudTypeMap(){
    try{
      ...
      const previousTypeMap=typeMap,previousSaveTypeMap=saveTypeMap;
      typeMap=function(){return Object.assign({},cloudOverrides,previousTypeMap())};
      saveTypeMap=function(map){previousSaveTypeMap(map);cloudOverrides=Object.assign({},cloudOverrides,map||{});persistCloud(cloudOverrides)};
      patched=true;return true;
    }catch(e){return false}
  }
  async function syncCloudOverrides(){
    if(busy)return false;const u=currentUser();if(!u||typeof sb==='undefined'||!sb)return false;if(!installCloudTypeMap())return false;busy=true;
    try{
      const local=localMap(u.id),before=JSON.stringify(typeMap()),r=await sb.from('settings').select('overrides').eq('user_id',u.id).maybeSingle();
      if(r.error){busy=false;return false}
      const cloud=(r.data&&r.data.overrides&&typeof r.data.overrides==='object')?r.data.overrides:{},merged=Object.assign({},cloud,local);
      cloudOverrides=merged;writeLocal(u.id,merged);if(JSON.stringify(cloud)!==JSON.stringify(merged))await persistCloud(merged);
      const after=JSON.stringify(typeMap());busy=false;if(before!==after&&typeof render==='function')render();return true;
    }catch(e){busy=false;return false}
  }
  const timer=setInterval(async function(){syncCount++;const ok=await syncCloudOverrides();if((ok&&syncCount>=6)||syncCount>30)clearInterval(timer)},500);
  window.addEventListener('focus',syncCloudOverrides);document.addEventListener('visibilitychange',function(){if(!document.hidden)syncCloudOverrides()});
})();

(function(){
  function applyBurntOrangeAccent(){
    if(document.getElementById('fintrack-accent-v34'))return;
    const style=document.createElement('style');style.id='fintrack-accent-v34';style.textContent=`:root{--gold:#B56E3B!important;--gold2:#C48357!important}body.dark{--gold:#B56E3B!important;--gold2:#C48357!important}.btn.gold{background:#B56E3B!important;color:#fff!important}.btn.gold:hover{background:#9C5A2F!important}.progress i{background:#B56E3B!important}.bottom button.active{color:#B56E3B!important}`;document.head.appendChild(style);
  }
...
/* Load the current audit directly from runtime-config so hard refresh does not depend on SW HTML injection. */
(function(){
  const s=document.createElement('script');s.src='./savings-audit-v42.js?v=44';s.defer=true;document.head.appendChild(s);
})();

/* Reveal only after current layout + savings wrappers are installed. */
(function(){let n=0,t=setInterval(function(){n++;if((window.__fintrackSavingsInstalled&&window.__fintrackLayoutV32)||n>300){document.getElementById('fintrack-v44-boot')?.remove();clearInterval(t)}},20)})();