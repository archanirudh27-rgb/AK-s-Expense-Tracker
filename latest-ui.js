(function(){
  ...
  /* Brand styling — refined F monogram + gold FinTrack wordmark. */
  ...
  function setBrand(){
    ...
    document.querySelectorAll('.brand').forEach(function(el){
      if(!el.querySelector('.fin-brand-lockup')){
        el.innerHTML='<span class="fin-brand-lockup"><img class="fin-brand-icon" src="./icon-192.svg?v=22" alt="FinTrack logo"><span class="fin-wordmark">Fin<span class="fin-word-gold">Track</span></span></span>';
      }
    });
  }
  ...
  function addHomeLogo(){
    ...
    box.innerHTML='<img src="./icon-192.svg?v=22" alt="FinTrack"><div><div class="fin-home-title">Fin<span class="fin-word-gold">Track</span></div><div class="fin-home-sub">'+TAGLINE+'</div></div>';
  ...
  /* Keep the user on the same app tab after reload. */
  const allowedPages=['home','tx','add','forecast','settings'];
  if(typeof window.go==='function'&&!window.__fintrackGoPatched){
    ...
    if(saved&&allowedPages.includes(saved))originalGo(saved);
  }
  ...
  var fs=document.createElement('script');fs.src='./forecast.js?v=22';document.head.appendChild(fs);
})();
