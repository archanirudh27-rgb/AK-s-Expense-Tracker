/* v73 Money Back source mapping + net expense type reconciliation. */
(function(){
  let installed=false;
  ...
  function install(){
    ...
    byType=function(a){const m={'Fixed Recurring':0,'Variable Recurring':0,'Non-Recurring':0,'Uncertain':0};(a||[]).filter(x=>ordinary(x)).forEach(x=>{const t=typeFor(x);if(t in m)m[t]+=Number(x.amount||0)});backsFor(a).forEach(x=>{const t=parse(x).type;if(t in m)m[t]=Math.max(0,m[t]-Number(x.amount||0))});return m};
