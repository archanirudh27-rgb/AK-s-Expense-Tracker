const CACHE_NAME='fintrack-pwa-v76';
...
self.addEventListener('fetch',event=>{
  ...
  if(url.pathname.endsWith('/runtime-config.js')){
    event.respondWith(Promise.all([
      ...
      fetch('./v73.js?v=76',{cache:'no-store'}).then(r=>r.text()),
      fetch('./v76.js?v=76',{cache:'no-store'}).then(r=>r.text())
    ]).then(([base,v73,v76])=>new Response(base+'\n'+v73+'\n'+v76,{headers:{'Content-Type':'application/javascript; charset=utf-8','Cache-Control':'no-store'}})));
    return;
  }
