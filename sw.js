const CACHE_NAME='fintrack-pwa-v40';
const SHELL=['./','./index.html','./manifest.webmanifest?v=35','./runtime-config.js?v=34','./balance-fix-v40.js?v=40','./fintrack-icon-192.svg?v=35','./fintrack-icon-512.svg?v=35'];
...
self.addEventListener('fetch',event=>{
  ...
  if(event.request.mode==='navigate'){
    event.respondWith(fetch(event.request,{cache:'no-store'}).then(async response=>{
      ...
      const withoutOld=text.replace(/<script src="\.\/balance-fix-v(?:37|38|39)\.js\?v=(?:37|38|39)"><\/script>/g,'');
      const injected=withoutOld.includes('balance-fix-v40.js')?withoutOld:withoutOld.replace('</body>','<script src="./balance-fix-v40.js?v=40"></script></body>');
