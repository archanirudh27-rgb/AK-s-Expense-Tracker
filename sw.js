const CACHE_NAME='fintrack-pwa-v75';
...
self.addEventListener('fetch',event=>{
  ...
  if(url.pathname.endsWith('/runtime-config.js')){
    event.respondWith(Promise.all([
      ...
      fetch('./v73.js?v=75',{cache:'no-store'}).then(r=>r.text()),
      fetch('./v75.js?v=75',{cache:'no-store'}).then(r=>r.text())
    ]).then(([base,v73,v75])=>new Response(base+'\n'+v73+'\n'+v75,{headers:{'Content-Type':'application/javascript; charset=utf-8','Cache-Control':'no-store'}})));
    return;
  }
