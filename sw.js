const CACHE_NAME='fintrack-pwa-v33';
const SHELL=['./','./index.html','./manifest.webmanifest?v=33','./runtime-config.js?v=33','./fintrack-icon-192.svg?v=29','./fintrack-icon-512.svg?v=29'];
...
self.addEventListener('fetch',event=>{
  ...
  if(url.pathname.endsWith('/runtime-config.js')){
    event.respondWith(fetch('./runtime-config.js?v=33',{cache:'no-store'}).then(response=>{const copy=response.clone();caches.open(CACHE_NAME).then(cache=>cache.put(event.request,copy));return response}).catch(()=>caches.match(event.request)));
    return;
  }
