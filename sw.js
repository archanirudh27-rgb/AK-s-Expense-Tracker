const CACHE_NAME='fintrack-pwa-v74';
...
self.addEventListener('fetch',event=>{
  ...
  if(url.pathname.endsWith('/runtime-config.js')){
    event.respondWith(Promise.all([fetch(event.request,{cache:'no-store'}).then(r=>r.text()),fetch('./v73.js?v=74',{cache:'no-store'}).then(r=>r.text())]).then(([base,patch])=>new Response(base+'\n'+patch,{headers:{'Content-Type':'application/javascript; charset=utf-8','Cache-Control':'no-store'}})));
    return;
  }
